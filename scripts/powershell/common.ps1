...
function Test-FeatureBranch {
    ...
    if ($Branch -notmatch '^(feature/([A-Z0-9]+-)?)?[0-9]{3}-' -and $Branch -notmatch '^\d{8}-\d{6}-') {
        Write-Output "ERROR: Not on a feature branch. Current branch: $Branch"
        Write-Output "Feature branches should be named like: feature/001-feature-name, feature/URA-001-feature-name, or 20260319-143022-feature-name"
        return $false
    }
    return $true
}
